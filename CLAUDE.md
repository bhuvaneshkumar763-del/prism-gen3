...
**Recommended next step**: Phase 3 (settings) — a multi-tab options page
covering the `bubble*`/`sourceLanguageByHost`/`hoverTooltipEnabled`/
`selectionPopupEnabled` keys this pass and Phase 1 both added, plus backup
export/import and restore-defaults (`configStore.export()`/`import()`
already exist and are already tested, just not wired to any UI yet).
