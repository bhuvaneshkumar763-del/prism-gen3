...
| Safari build (macOS + iOS/iPadOS) | New, `0.3.0-beta.56` | `safari/Prism/` — a real Xcode project (Apple's `safari-web-extension-converter`) wrapping the same `wxt build -b safari` output the other two targets use. Both macOS and iOS builds concretely verified (real code-signed macOS build, real iOS Simulator install+launch+screenshot), not just assumed to compile. `npm run safari:sync` keeps it current after engine/UI changes; see `safari/README.md` for the full setup, the persistent-background-page fix (iOS needs `background.persistent: false`, WXT's shared MV2 manifest defaults to `true`), and what genuinely needs the developer's own Apple account (distribution, App Store submission) rather than being an agent-side gap. Not wired into CI — CI runs on `ubuntu-latest`, and a Safari build needs a `macos-latest` runner; a deliberate scope cut for this pass, not an oversight.
